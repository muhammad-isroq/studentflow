                    ...
                    ->schema([
                        ...
                        RepeatableEntry::make('summary_data')
                            ...
                            ->state(function ($livewire) {
                                ...
                                // MODIFIKASI: Ambil siswa aktif DAN siswa yang sudah pindah tapi punya nilai di sini
                                $students = \App\Models\Siswa::query()
                                    ->where('program_id', $livewire->program_id)
                        ...
                        Grid::make(9)
                            ...
                            ->schema([
                                ...
                                TextEntry::make('c_l')->hiddenLabel()->alignCenter()->weight(FontWeight::Bold)->state(fn ($livewire) => $this->calcClassAvg($livewire, 'listening')),
                                TextEntry::make('c_r')->hiddenLabel()->alignCenter()->weight(FontWeight::Bold)->state(fn ($livewire) => $this->calcClassAvg($livewire, 'reading')),
                                TextEntry::make('c_w')->hiddenLabel()->alignCenter()->weight(FontWeight::Bold)->state(fn ($livewire) => $this->calcClassAvg($livewire, 'writing')),
                                TextEntry::make('c_s')->hiddenLabel()->alignCenter()->weight(FontWeight::Bold)->state(fn ($livewire) => $this->calcClassAvg($livewire, 'speaking')),
                                TextEntry::make('c_g')->hiddenLabel()->alignCenter()->weight(FontWeight::Bold)->state(fn ($livewire) => $this->calcClassAvg($livewire, 'grammar')),
                                TextEntry::make('c_total')->hiddenLabel()->alignCenter()->weight(FontWeight::Black)->color('primary')
                                    ->state(function ($livewire) {
                                        $total = (float)$this->calcClassAvg($livewire, 'listening') + (float)$this->calcClassAvg($livewire, 'reading') + (float)$this->calcClassAvg($livewire, 'writing') + (float)$this->calcClassAvg($livewire, 'speaking') + (float)$this->calcClassAvg($livewire, 'grammar');
    ...
    public function calcClassAvg($livewire, $column)
{
    ...
    // MODIFIKASI: Ambil ID semua siswa yang aktif ATAU pernah punya nilai di program ini
    $relevantStudentIds = \App\Models\Siswa::query()
        ->where('program_id', $program->id)
        ->orWhereHas('grades', function ($q) use ($assessments) {
            $q->whereIn('assessment_id', $assessments->pluck('id'));
        })
        ->pluck('id');

    $avgReviews = \App\Models\Grade::whereIn('student_id', $relevantStudentIds)
                        ->whereIn('assessment_id', $reviewIds)
                        ->avg($column) ?? 0;

    $avgSemester = 0;
    if ($semesterTestId) {
        $avgSemester = \App\Models\Grade::whereIn('student_id', $relevantStudentIds)
                ...
                ->where(function ($query) {
                    $query->where('program_id', $this->program_id)
                        ->orWhereHas('grades', function ($q) {
                            $q->whereHas('assessment', function ($aq) {
                                $aq->where('program_id', $this->program_id);
                            });
                        });
                })
            )
            ->columns(array_merge(
                [
                    Tables\Columns\TextColumn::make('nama')
                        ...
                        ->sortable()
                        ->searchable()
                        ->weight('bold')->description(fn (Siswa $record) => 
                        $record->program_id != $this->program_id 
                            ? "Moved to: " . ($record->program->nama_program ?? 'Other Program') 
                            : null
                    ),
                ],
